    ...
    if (qisActive) {
      if (!validateIsActive(Boolean(qisActive))) {
        filteredArray = filteredArray.filter(
          (user) => String(user.isActive) == qisActive
        );
      } else {
        return res.status(404).json({ message: "invaild entry" });
      }
...
export const getUserById = (req, res) => {
  ...
  if (userExists) {
    return res
      .status(200)
      .json({ userData: users.find((user) => user.id == uID) });
  } else {
...
export const addUser = (req, res) => {
  ...
  if (!name || !email || !age || !role || !isActive) {
    res
      ...
      .json({
        message:
          "Please fill all the input for creating a new user \n eg:- name,email,age,role,isActive",
      });
  } else if (!validateEmail(email)) {
    res.status(404).json({ message: "Invalid email !!" });
  } else if (validateAge(age)) {
    res.status(404).json({ message: "Invalid age !!" });
  } else if (validateRole(role)) {
    res.status(404).json({ message: "Invalid Role must be admin or user!!" });
    ...
    const new_users = `
        const users = [
            ${users.map((user) => JSON.stringify(user)).Yupn(",\n    ")}
            ];
